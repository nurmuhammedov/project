import {isNotFutureDate} from 'helpers/yup'
...
export const expenseTransactionSchemaNoCustomer = yup.object().shape({
	store: yup.number().required('This field is required'),
	records: yup
		.array()
		.default([])
		...
		.required('This field is required'),
	currency: yup.string().trim().required('This field is required'),
	service_type: yup.string().trim().required('This field is required'),
	first_amount: yup.string().trim().required('This field is required'),
	type: yup.number().oneOf([3]).required(),
	date: isNotFutureDate.required('This field is required'),
	description: yup
		.string()
		.trim()
		.optional()
